...
class bb_Meta_Box {
	...
	static function js_css() {
		...
		wp_enqueue_style('rw-meta-box', $base_url . '/taxonomy.css');
		wp_enqueue_script('rw-meta-box', $base_url . '/taxonomy.js', array('jquery'), null, true);
	}
	...
	function check_field_upload() {
		...
		add_action('wp_ajax_bb_delete_file', array(&$this, 'delete_file'));			// ajax delete files
		add_action('wp_ajax_bb_reorder_images', array(&$this, 'reorder_images'));	// ajax reorder images
	}
	...
	function insert_images() {
		...
		$nonce = wp_create_nonce('bb_ajax_delete');
	...
	function delete_file() {
		...
		if (!wp_verify_nonce($nonce, 'bb_ajax_delete')) die('1');
	...
	function reorder_images() {
		...
		if (!wp_verify_nonce($nonce, 'bb_ajax_reorder')) die('1');
	...
	function show_field_text($field, $meta) {
		...
		echo "<input type='text' class='rw-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='50' />";
	...
	function show_field_image($field, $meta) {
		...
		$nonce_delete = wp_create_nonce('bb_ajax_delete');
		$nonce_sort = wp_create_nonce('bb_ajax_reorder');
