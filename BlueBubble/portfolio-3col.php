<?php
/*
Template Name: Portfolio: 3 Column Sidebar
*/
?>
...
<?php
	if ( get_option('bb_portfolio_num') =='true' ) {  $portfolio_num = get_option('bb_portfolio_num'); 
...
<?php $bb_portfolio_cat = get_option('bb_portfolio_cat');  ?>
    ...
    <?php query_posts('paged='.$paged.'&category_name='.$bb_portfolio_cat.'&posts_per_page='.$portfolio_num); ?>
        <?php while (have_posts()) : the_post(); ?>
            ...
            <div class="post-3col" id="post-<?php the_ID(); ?>">
                ...
                <?php }else{ ?>
                <?php if (has_post_thumbnail()=='') { ?><img width='215' height='104' src="<?php echo bloginfo('template_directory'); ?>/images/no-img/no-img657x318.png" /><?php } else { ?>
                <?php $thumbID = get_post_thumbnail_id($post->ID); ?>
                <a href="<?php echo wp_get_attachment_url($thumbID); ?>" rel="project">
				<span class="roll-3col" ></span><?php } // End check for No Colorbox ?>
         		<?php echo the_post_thumbnail('3col-thumb'); ?>
				</a><?php } ?>
